...
async function resolveRequest(actor) {
  ...
  try {
    ...
    const result = await DialogV2.wait({
      ...
      content,
      buttons: [
        { icon: 'fas fa-check', label: _loc('SPELLBOOK.Approval.Approve'), action: 'approve', callback: (_event, button) => (new FormData(button.form).get('skipTime') ? 'skip' : 'approve') },
        { icon: 'fas fa-times', label: _loc('SPELLBOOK.Approval.Deny'), action: 'deny' }
      ],
      default: 'approve',
      rejectClose: false
    });
